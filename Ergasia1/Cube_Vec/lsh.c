...
void lsh(struct vec *vectors, struct h_func **h, int *m_factors, struct projection *p, int vec_sum, int coords, int M, int k, int L, int w){
	...
	for(i=0; i<vec_sum; i++){
		for(z=0; z<L; z++){
			...
			p[i].g[z] = concut(h[z], k);				//Thelei diorthwsi
		}
	}
}
